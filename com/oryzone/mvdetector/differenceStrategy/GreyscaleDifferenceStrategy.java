...
public class GreyscaleDifferenceStrategy extends AbstractDifferenceStrategy
{
	...
	IplImage prevImageGS = IplImage.create(prevImage.width(), prevImage.height(), IPL_DEPTH_8U, 1);
        cvCvtColor(prevImage, prevImageGS, CV_RGB2GRAY);
        
        IplImage currImageGS = IplImage.create(currImage.width(), currImage.height(), IPL_DEPTH_8U, 1);
	...
	for(int y = 0; y < height; y++)
	{
		for(int x = 0; x < width; x++)
		{
			...
			Color color = new Color(	Math.max(0, Math.min(255, cColor + dColor)), 
							Math.max(0, Math.min(255, cColor + dColor)), 
							Math.max(0, Math.min(255, cColor + dColor ))
						);
